...
void CommonNetworkApi::set_event_queue(std::shared_ptr<EventQueue> event_queue_ptr) noexcept {
...
void CommonNetworkApi::process_chunk_arrival(void* args) noexcept {
    ...
    // parse chunk data
    auto* const data = static_cast<std::tuple<int, int, int, uint64_t, int>*>(args);
...
void CommonNetworkApi::sim_schedule(const timespec_t delta, void (*fun_ptr)(void*), void* const fun_arg) {
