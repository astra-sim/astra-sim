...
static void exit_error(char* progr) {
    std::cout << "Usage " << progr
              << " [UNCOUPLED(DEFAULT)|COUPLED_INC|FULLY_COUPLED|COUPLED_"
                 "EPSILON] [epsilon][COUPLED_SCALABLE_TCP"
              << std::endl;
    exit(1);
}
...
HTSimProtoTcp::HTSimProtoTcp(const HTSim::tm_info* const tm,
                             int argc,
                             char** argv) {
    ...
    sinkLogger =
        std::make_unique<TcpSinkLoggerSampling>(timeFromMs(1000), eventlist);
    logfile->addLogger(*sinkLogger);

    tcpRtxScanner =
        std::make_unique<TcpRtxTimerScanner>(timeFromMs(10), eventlist);
    ...
    if (topo_file) {
        ...
        FatTreeTopology* top_ = FatTreeTopology::load(
            topo_file, qlf.get(), eventlist, memFromPkt(8), RANDOM, FAIR_PRIO);
        ...
        if (top->no_of_nodes() != no_of_nodes) {
            std::cerr << "Mismatch between connection matrix (" << no_of_nodes
                      << " nodes) and topology (" << top->no_of_nodes()
                      << " nodes)" << endl;
            exit(1);
        }
    } else {
        top = std::make_unique<FatTreeTopology>(no_of_nodes, linkspeed,
                                                memFromPkt(8), qlf.get(),
                                                &eventlist, ff, RANDOM, 0);
    }
#endif

#ifdef OV_FAT_TREE
    top = std::make_unique<OversubscribedFatTreeTopology>(logfile.get(),
                                                          &eventlist, ff);
#endif

#ifdef MH_FAT_TREE
    top = std::make_unique<MultihomedFatTreeTopology>(logfile.get(), &eventlist,
                                                      ff);
#endif
...
void HTSimProtoTcp::schedule_htsim_event(FlowInfo flow, int flow_id) {
    ...
    it_sub = crt_subflow_count > net_paths[src][dst]->size()
                 ? net_paths[src][dst]->size()
                 : crt_subflow_count;
    ...
    for (uint32_t inter = 0; inter < it_sub; inter++) {
        ...
        tcpSrc->setName("mtcp_" + ntoa(src) + "_" + ntoa(inter) + "_" +
                        ntoa(dst));
...
#if PRINT_PATHS
        paths << "Route from " << ntoa(src) << " to " << ntoa(dst) << "  ("
              << choice << ") -> ";
        ...
        tcpSrc->connect(*routeout, *routein, *tcpSnk,
                        start + timeFromMs(extrastarttime));
