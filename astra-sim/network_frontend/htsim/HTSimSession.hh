...
enum class HTSimProto { None, Tcp };
...
struct HTSimConf {
    // When this option is true, a flow will be marked as finished for the
    // receiver as soon as the last packet is received instead of waiting for
    // sender to get ack.
...
class MsgEvent {
  public:
    ...
    // from maps such as send_waiting, we should always check that a MsgEvent
    // exists for the given key. (i.e. this default constructor should not be
    // called in runtime.)
    MsgEvent()
        : src_id(0),
          dst_id(0),
...
class HTSimSession {
  public:
    ...
    static HTSimSession& init(const HTSim::tm_info* const tm,
                              int argc,
                              char** argv,
    ...
    double get_time_ns();
    double get_time_us();
    void schedule_astra_event(long double delta,
                              EventHandler msg_handler,
                              void* fun_arg);

    // Used to count how many bytes were sent/received by this node.
    // Refer to sim_finish().
    //   - key: Pair <node_id, send/receive>. Where 'send/receive' indicates if
    //   the value is for send or receive
    ...
    //          TODO: Adding port_id as key is a hacky solution. The real
    //          solution would be to split this map, similar to recv_waiting and
    //          msg_standby.
    //   - value: A MsgEvent instance that indicates that Sys layer is waiting
    //   for a send event to finish
    static std::map<std::pair<HTSim::MsgEventKey, int>, HTSim::MsgEvent>
        send_waiting;

    //   - recv_waiting holds messages where sim_recv has been called but HTSim
    //   has not yet simulated the message arriving,
    //   - msg_standby holds messages which HTSim has simulated the arrival, but
    //   sim_recv has not yet been called.

    //   - key: A MsgEventKey instance.
    //   - value: A MsgEvent instance that indicates that Sys layer is waiting
    //   for a receive event to finish
    static std::map<HTSim::MsgEventKey, HTSim::MsgEvent> recv_waiting;

    //   - key: A MsgEventKey instance.
    //   - value: The number of bytes that HTSim has simulated completed, but
    //   the System layer has not yet called sim_recv
    static std::map<HTSim::MsgEventKey, int> msg_standby;

    static std::map<int, int> flow_id_to_tag;

    static void notify_receiver_receive_data(
        int src_id, int dst_id, int message_size, int tag, int flow_id);
    static void notify_sender_sending_finished(
        int src_id, int dst_id, int message_size, int tag, int flow_id);
  ...
  private:
    // Singleton pattern
    HTSimSession(const tm_info* const tm,
                 int argc,
                 char** argv,
                 HTSimProto proto);
    ~HTSimSession();
