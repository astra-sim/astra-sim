...
public:
  ...
  WrapperRelayData (FastBackEnd *fast_backend, Type type, double creation_time, int partner_node,
                    uint64_t comm_size, void (*msg_handler) (void *fun_arg), void *fun_arg);
};
...
public:
  ...
  /**
     ...
     */
  void insert (int src, int dest, int tag, int communicationSize, WrapperData::Type simulationType);
  ...
  /**
     ...
     * @param tag
     * @param communicationSize
     * @return <true, WrapperData::Type> if matching pair exists, return <false, Undefined> if not.
     */
  std::pair<bool, WrapperData::Type> pop (int src, int dest, int tag, int communicationSize);
...
public:
  ...
  /**
     ...
     * @param latency
     */
  void insertLatencyData (std::pair<int, int> nodesPair, int communicationSize, double latency);
  ...
  /**
     ...
     */
  std::pair<bool, double> lookupLatency (std::pair<int, int> nodesPair, int communicationSize);
  ...
  /**
     * Returns a boolean whether there's enough number of data points available for prediction.
