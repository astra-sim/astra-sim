...
namespace AstraSim{
WrapperData::WrapperData (WrapperData::Type type, void (*msg_handler) (void *fun_arg),
                          void *fun_arg)
{
  this->type = type;
...
WrapperRelayData::WrapperRelayData (FastBackEnd *fast_backend, WrapperData::Type type,
                                    double creation_time, int partner_node, uint64_t comm_size,
...
void
InflightPairsMap::insert (int src, int dest, int tag, int communicationSize,
                          WrapperData::Type simulationType)
{
  auto insertionResult =
      inflightPairs.emplace (std::make_tuple (src, dest, tag, communicationSize), simulationType);

  // Insertion should success, i.e., there shouldn't be the same pair already in the map
  assert (insertionResult.second);
}

std::pair<bool, WrapperData::Type>
InflightPairsMap::pop (int src, int dest, int tag, int communicationSize)
{
  auto searchResult = inflightPairs.find (std::make_tuple (src, dest, tag, communicationSize));
...
void
InflightPairsMap::print ()
{
  for (const auto &pair : inflightPairs)
    {
      std::cout << "src: " << std::get<0> (pair.first) << ", dest: " << std::get<1> (pair.first)
                << ", tag: " << std::get<2> (pair.first)
                << ", communicationSize: " << std::get<3> (pair.first) << std::endl;
    }
}

void
DynamicLatencyTable::insertLatencyData (std::pair<int, int> nodesPair, int communicationSize,
                                        double latency)
{
  // 1. retrieve table
  ...
  auto latencyInsertionResult = latencyTable->second.emplace (communicationSize, latency);

  // 3. Check whether insertion happened normally -- i.e., check whether communicationSize was unique
...
DynamicLatencyTable::lookupLatency (std::pair<int, int> nodesPair, int communicationSize)
{
...
double
DynamicLatencyTable::predictLatency (std::pair<int, int> nodesPair, int communicationSize)
{
...
bool
DynamicLatencyTable::canPredictLatency (std::pair<int, int> nodesPair)
{
...
void
DynamicLatencyTable::print ()
{
  for (const auto &latencyTable : latencyTables)
    {
      std::cout << "src: " << latencyTable.first.first << ", dest: " << latencyTable.first.second
                << ", datapoints: " << latencyDataCountTable[latencyTable.first] << std::endl;
      for (const auto &latencyPair : latencyTable.second)
        {
          std::cout << "\t- commSize: " << latencyPair.first << " - latency: " << latencyPair.second
                    << std::endl;
        }
    }
}
...
void
DynamicLatencyTable::insertNewTableForNode (std::pair<int, int> nodesPair)
{
  // Make sure tables doesn't exist.
  auto latencyTable = latencyTables.find (nodesPair);
  auto latencyDataCount = latencyDataCountTable.find (nodesPair);
  assert ((latencyTable == latencyTables.end ()) &&
          (latencyDataCount == latencyDataCountTable.end ()));

  // Insert new node table
  auto latencyTableInsertionResult = latencyTables.emplace (nodesPair, std::map<int, double> ());
  auto latencyDataCountInsertionResult = latencyDataCountTable.emplace (nodesPair, 0);
  assert (latencyTableInsertionResult.second && latencyDataCountInsertionResult.second);
}
...
void
FastBackEnd::handleEvent (void *arg)
{
  ...
  else if (((WrapperRelayData *) wrapperData)->type == WrapperData::Type::DetailedRecv)
    { //should be replaced by type==WrapperData::Type::DetailedRecv
      WrapperRelayData *wrapperRelayData = (WrapperRelayData *) arg;
      timespec_t current_time = wrapperRelayData->fast_backend->wrapped_backend->sim_get_time ();
      wrapperRelayData->fast_backend->update_table_recv (
          wrapperRelayData->creation_time, current_time.time_val, wrapperRelayData->partner_node,
          wrapperRelayData->comm_size);

      (*(wrapperRelayData->msg_handler)) (wrapperRelayData->fun_arg);
      delete wrapperRelayData;
    }
  else if (((WrapperRelayData *) wrapperData)->type == WrapperData::Type::DetailedSend)
    { //should be replaced by type==WrapperData::Type::DetailedSend
      WrapperRelayData *wrapperRelayData = (WrapperRelayData *) arg;
      timespec_t current_time = wrapperRelayData->fast_backend->wrapped_backend->sim_get_time ();
...
FastBackEnd::FastBackEnd (int rank, AstraNetworkAPI *wrapped_backend) : AstraNetworkAPI (rank)
{
  this->wrapped_backend = wrapped_backend;
}

double
FastBackEnd::sim_time_resolution ()
{
  return wrapped_backend->sim_time_resolution ();
}

int
FastBackEnd::sim_finish ()
{
...
int
FastBackEnd::sim_comm_size (sim_comm comm, int *size)
{
  return wrapped_backend->sim_comm_size (comm, size);
}

timespec_t
FastBackEnd::sim_get_time ()
{
  return wrapped_backend->sim_get_time ();
}

int
FastBackEnd::sim_init (AstraMemoryAPI *MEM)
{
  return wrapped_backend->sim_init (MEM);
}

void
FastBackEnd::sim_schedule (timespec_t delta, void (*fun_ptr) (void *fun_arg), void *fun_arg)
{
...
void
FastBackEnd::update_table_send (double start, double finished, int dst, uint64_t comm_size)
{
  auto latency = finished - start;
  auto src = sim_comm_get_rank ();
  dynamicLatencyTable.insertLatencyData (std::make_pair (src, dst), comm_size, latency);
}

void
FastBackEnd::update_table_recv (double start, double finished, int src, uint64_t comm_size)
{
  auto latency = finished - start;
                                 ...
                                 void *fun_arg)
{
  ...
  WrapperRelayData *wrapperData =
      new WrapperRelayData (this, WrapperData::Type::DetailedRecv, current_time.time_val, src,
                            count, msg_handler, fun_arg);
  return wrapped_backend->sim_recv (buffer, count, type, src, tag, request,
                                    &FastBackEnd::handleEvent, wrapperData);
}
                                 ...
                                 void *fun_arg)
{
  timespec_t current_time = wrapped_backend->sim_get_time ();
  WrapperRelayData *wrapperData =
      new WrapperRelayData (this, WrapperData::Type::DetailedSend, current_time.time_val, dst,
                            count, msg_handler, fun_arg);
  return wrapped_backend->sim_send (buffer, count, type, dst, tag, request,
                                    &FastBackEnd::handleEvent, wrapperData);
}

int
FastBackEnd::fast_send_recv_request (double delay, void (*msg_handler) (void *fun_arg),
                                     void *fun_arg)
{
  timespec_t delta;
  ...
  wrapped_backend->sim_schedule (delta, &FastBackEnd::handleEvent, (void *) wrapperData);
  return 1;
}
        ...
        case WrapperData::Type::FastSendRecv:
          {
            ...
            auto predictedLatency = dynamicLatencyTable.predictLatency (srcDestPair, count);
        ...
        case WrapperData::Type::DetailedRecv:
          {
            ...
            return relay_send_request (buffer, count, type, dst, tag, request, msg_handler,
                                       fun_arg);
          }
  ...
  if (lookupResult.first)
    {
      ...
      inflightPairsMap.insert (src, dst, tag, count, WrapperData::Type::FastSendRecv);
      return fast_send_recv_request (lookupResult.second, msg_handler, fun_arg);
    }
  ...
  if (dynamicLatencyTable.canPredictLatency (srcDestPair))
    {
      ...
      if ((rand () % 100) < 10)
        {
          inflightPairsMap.insert (src, dst, tag, count, WrapperData::Type::DetailedSend);
          return relay_send_request (buffer, count, type, dst, tag, request, msg_handler, fun_arg);
        }
  ...
  if (inflightPair.first)
    {
      ...
      switch (inflightPair.second)
        ...
        case WrapperData::Type::DetailedSend:
          {
            // relay to ns3
            return relay_recv_request (buffer, count, type, src, tag, request, msg_handler,
                                       fun_arg);
          }
  ...
  if (dynamicLatencyTable.canPredictLatency (srcDestPair))
    {
      ...
      if ((rand () % 100) < 10)
        {
          inflightPairsMap.insert (src, dst, tag, count, WrapperData::Type::DetailedRecv);
          return relay_recv_request (buffer, count, type, src, tag, request, msg_handler, fun_arg);
        }
