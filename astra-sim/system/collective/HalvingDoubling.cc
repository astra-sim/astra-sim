    ...
    HalvingDoubling::HalvingDoubling(
        ...
        switch (type) {
            ...
            case ComType::Reduce_Scatter:
                ...
                break;
            default:
                std::cerr << "######### Exiting because of unknown communication type for HalveingDoubling collective algorithm #########" <<std::endl;
                std::exit(1);
        }
        RingTopology::Direction direction=specify_direction();
        this->current_receiver=id;
        for(int i=0;i<rank_offset;i++){
            this->current_receiver = ring_topology->get_receiver_node(this->current_receiver, direction);
