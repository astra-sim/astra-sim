...
#include "astra-sim/system/basic_system/astraccl/CollectiveImplLookup.hh"
...
CollectiveImpl* generate_collective_impl_from_input(
    ...
    if (collective_impl_str == "ring") {
        ...
        logger->critical(
            "Cannot interpret collective implementations. "
...
std::map<int, std::string> parse_per_node_yaml_file(string yaml_filepath) {
    ...
    if (!root || !root.IsMap()) {
        throw std::runtime_error(
            "Top-level YAML must be a mapping of int -> string.");
    }
    ...
    for (const auto& kv : root) {
        if (!kv.first.IsScalar() || !kv.second.IsScalar()) {
            throw std::runtime_error(
                "YAML mapping keys and values must be scalars.");
        }
        int node_id;
        try {
...
void CollectiveImplLookup::setup_collective_impl_from_config(json j) {
    ...
    // 2. Parse the custom collectives to be applied on all collectives, if
    // defined. These have the next highest priority.
    ...
    // 3. Finally, parse the list of per-chakra-node custom collective
    // algorithm. These have the highest priority.
    if (j.contains("per-node-custom-implementation")) {
        string per_node_custom_impl_filepath =
            j["per-node-custom-implementation"];
        std::map<int, std::string> per_node_custom_impl_filename =
            parse_per_node_yaml_file(per_node_custom_impl_filepath);

        for (auto const& [node_id, chakra_filepath] :
             per_node_custom_impl_filename) {
            CollectiveImpl* ci =
...
std::vector<CollectiveImpl*> CollectiveImplLookup::get_collective_impl(
    ComType comm_type, uint64_t workload_node_id, BypassRule bypass_rule) {
    ...
    // Next, check if there is a global custom implementation for this
    // collective type.
    ...
    // Traditional native, it was okay to not define a collective in system
    // input as long as that collective was not actually present. Still,
    // GeneralLogicalTopo was utilized for all collectives.
    if (bypass_rule == BypassRule::BYPASS_ALL_CUSTOM) {
        return {};
    }
    ...
    throw std::runtime_error(
        "No collective implementation found for the given type {" +
        std::to_string(static_cast<int>(comm_type)) + "} and node ID {" +
        std::to_string(workload_node_id) + "}.");
}
