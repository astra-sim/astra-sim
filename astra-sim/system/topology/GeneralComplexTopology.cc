...
GeneralComplexTopology::GeneralComplexTopology(int id,
    ...
    for (uint64_t dim = 0; dim < collective_impl.size(); dim++) {
        if (collective_impl[dim]->type == CollectiveImplType::Ring ||
            ...
            // While executing a collective according a Chakra ET representation does not need information on the
            // logical topology, The system layer's logic of defining and invoking "collective phase" objects (which in
            // turn executes the individual collective algorithm implementation) does rely on the existence (not the
            // values) of the logical topology. (Refer to functions involving 'Sys.cc::generate_collective') Therefore,
            // we fill in the logical topology with a dummy, default value.
