...
namespace AstraSim{
    RingTopology::RingTopology(Dimension dimension,int id,int total_nodes_in_ring,int index_in_ring,int offset):
        ...
        if(id==0){
            std::cout<<"ring of node 0, "<<"id: "<<id<<" dimension: "<<name<<" total nodes in ring: "<<total_nodes_in_ring
                     <<" index in ring: "<<index_in_ring<<" offset: "<<offset<<"total nodes in ring: "<<total_nodes_in_ring<<std::endl;
        }
        this->id=id;
    ...
    int RingTopology::get_receiver_node(int node_id, Direction direction) {
        ...
        //std::cout<<"for node id of"<<node_id<<" at dimension: "<<name<<" index of: "<<index<<" is retrieved"<<std::endl;
        if(direction==RingTopology::Direction::Clockwise){
            ...
            if(receiver<0){
                std::cout<<"at dim: "<<name<<"at id: "<<id<<"dimension: "<<name<<" index: "<<index<<" ,node id: "<<node_id<<" ,offset: "<<offset<<
                         " ,index_in_ring: "<<index_in_ring<<" receiver: "<<receiver<<std::endl;
            }
            assert(receiver>=0);
