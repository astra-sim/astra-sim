...
CustomAlgorithm::CustomAlgorithm(std::string et_filename, int id, uint64_t data_size, int pos_in_comm, CommunicatorGroup* comm_group) : Algorithm() {
    try {
...
void CustomAlgorithm::issue(shared_ptr<Chakra::FeederV3::ETFeederNode> node) {
    ...
    int msg_chunk_cnt = get_msg_chunk_cnt(node);
        ...
        stream->owner->front_end_sim_send(
            ...
            comm_size, UINT8, dst_rank, node->comm_tag(),
        ...
        stream->owner->front_end_sim_recv(
            0, Sys::dummy_data, comm_size, UINT8, src_rank,
