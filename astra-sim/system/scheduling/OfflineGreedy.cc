...
std::vector<int> OfflineGreedy::get_chunk_scheduling(long long chunk_id,
    ...
    if (sys->id != 0) {
        return sys->all_sys[0]->offline_greedy->get_chunk_scheduling(chunk_id, remaining_data_size,
                                                                     recommended_chunk_size, dimensions_involved,
        ...
        for (auto& dim : dim_elapsed_time) {
            ...
            if (!dimensions_involved[dim.dim_num] || dim_size[dim.dim_num] == 1) {
                ...
                continue;
            } else if (inter_dim_scheduling == InterDimensionScheduling::OfflineGreedyFlex && !chunk_size_calculated) {
                chunk_size_calculated = true;
                if (comm_type == ComType::Reduce_Scatter) {
                    double load_difference = fabs(dim_elapsed_time.back().elapsed_time - dim.elapsed_time);
                    chunk_size = get_chunk_size_from_elapsed_time(load_difference, dim, ComType::Reduce_Scatter);
                } else {
                    ...
                    chunk_size = get_chunk_size_from_elapsed_time(load_difference, dim_elapsed_time[lastIndex],
                                                                  ComType::All_Gather);
                    lastIndex--;
                ...
                if (chunk_size < (recommended_chunk_size)) {
                    ...
                    for (uint64_t myDim = 0; myDim < dim_elapsed_time.size(); myDim++) {
                        for (uint64_t searchDim = 0; searchDim < dim_elapsed_time.size(); searchDim++) {
                            if (dim_elapsed_time[searchDim].dim_num == static_cast<uint64_t>(myDim)) {
                    ...
                    for (uint64_t myDim = 0; myDim < dim_elapsed_time.size(); myDim++) {
                        ...
                        if (comm_type == ComType::Reduce_Scatter) {
                            ...
                            dim_elapsed_time[myDim].elapsed_time +=
                                ((((double)chunk_size) / 1048576) * (((double)(dim_size[myDim] - 1)))) /
                                (dim_BW[myDim] / dim_BW[0]);
                            chunk_size *= dim_size[myDim];
                        }
                    }
