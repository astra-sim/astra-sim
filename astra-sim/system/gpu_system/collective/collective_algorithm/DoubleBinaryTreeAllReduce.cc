...
#include "astra-sim/system/gpu_system/gpu_model/PacketBundle.hh"
#include "astra-sim/system/gpu_system/gpu_model/RecvPacketEventHandlerData.hh"
...
void DoubleBinaryTreeAllReduce::run(EventType event, CallData* data) {
    if (state == State::Begin && type == BinaryTree::Type::Leaf) {  // leaf.1
        ...
        // arrived on the simulation side, in which case the callback would
        // enter the same state again.
