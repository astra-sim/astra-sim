...
void CSVWriter::initialize_csv(int rows, int cols) {
  auto logger = AstraSim::Logger::getLogger("CSVWriter");
  ...
  if (!myFile) {
    logger->critical("Unabled to open file: {}", path);
    logger->critical(
        "This error is fatal. Please make sure the CSV write path exists");
    exit(1);
  } else {
...
void CSVWriter::finalize_csv(
  ...
  if (!myFile) {
    logger->critical("Unable to open file: {}{}", path, name);
    logger->critical(
        "This error is fatal. Please make sure the CSV write path exists.");
    exit(1);
  } else {
    logger->info("success in opening file");
  }
  myFile.seekp(0, std::ios_base::beg);
  myFile.seekg(0, std::ios_base::beg);
...
void CSVWriter::write_cell(int row, int column, std::string data) {
  ...
  while (column > 0) {
    ...
    if (*buf == '\n') {
      auto logger = AstraSim::Logger::getLogger("CSVWriter");
      logger->critical("fatal error in inserting cell!");
      exit(1);
    }
  }
  str = str + data;
