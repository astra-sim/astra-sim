...
uint64_t SimpleMemory::npu_mem_read(uint64_t size){
    ...
    //std::cout<<"npu read req, node id: "<<NI->sim_comm_get_rank()<<", bytes: "<<size<<
    //", current delay: "<<delay<<", total requests: "<<npu_read_request_count<<std::endl;
    return delay;
...
uint64_t SimpleMemory::SimpleMemory::npu_mem_write(uint64_t size){
    ...
    //std::cout<<"npu write req, node id: "<<NI->sim_comm_get_rank()<<", bytes: "<<size<<
    //", current delay: "<<delay<<", total requests: "<<npu_write_request_count<<std::endl;
    return delay;
}
uint64_t SimpleMemory::nic_mem_read(uint64_t size){
    ...
    //", current delay: "<<delay<<", total read requests: "<<nic_read_request_count<<std::endl;
    return (uint64_t)(offset);
}
uint64_t SimpleMemory::nic_mem_write(uint64_t size){
    ...
    //", current delay: "<<delay<<", total write requests: "<<nic_write_request_count<<std::endl;
    return (uint64_t)(offset);
}
