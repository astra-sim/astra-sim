...
- **ns-3 libbgp submodule**: BGP/RDMA routing requires [Nat-Lab/libbgp](https://github.com/Nat-Lab/libbgp) (pinned 0.6.3) at `extern/network_backend/ns-3/deps/libbgp`. It is built automatically by CMake; no system `libbgp` install is needed. After cloning, run `git submodule update --init --recursive` (or `git submodule update --init deps/libbgp` inside the ns-3 tree).
