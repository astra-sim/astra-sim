...
- **Bundled protobuf/Abseil (default)**: C++ builds use `ASTRA_SIM_USE_BUNDLED_PROTOBUF=ON` (default) to fetch and build protobuf v29 with bundled Abseil via CMake FetchContent. No `PROTOBUF_FROM_SOURCE`, `absl_DIR`, or system `libprotobuf-dev` required for linking. First build downloads sources under `build/*/build/_deps/`. To use system protobuf 3.x instead: `cmake -DASTRA_SIM_USE_BUNDLED_PROTOBUF=OFF ..`
