...
function compile_chakra_et() {
  # C++ et_def sources are generated by CMake when ASTRA_SIM_USE_BUNDLED_PROTOBUF=ON (default).
  # Only generate the Python bindings here if missing (requires protoc on PATH).
  if [[ ! -f "${CHAKRA_ET_DIR:?}"/et_def_pb2.py ]] && command -v protoc >/dev/null 2>&1; then
    protoc et_def.proto \
